...
sapply(path_load, load, .GlobalEnv)
library("tidyverse")
```

## Introduction

As a follow-up to a previous post about correlations between Texas high school
...
```{r schools_tea_show_old, echo = FALSE, eval = FALSE}
schools_tea %>% 
  teproj::create_kable()
```
...
```{r schools_uil_show_old, echo = FALSE, eval = FALSE}
schools_uil %>% 
  teproj::create_kable()
```
...
```{r prettify_join_fuzzy, echo = FALSE}
...
Something that I did not consider here but is certainly worthy of exploration
is neural network text classificaton. With this approach, I could quantify
