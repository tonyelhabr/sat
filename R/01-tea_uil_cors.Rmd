...
```{r import_tea_data_funcs, results = "hide", eval = FALSE}
import_tea_data <-
  function(path = NULL, rgx_grp = NULL) {
    ...
    if(!is.null(rgx_grp)) {
      ret <-
        ret %>%
        filter(group %>% str_detect(rgp_rgx))
    }
    ret <-
      ret %>%
      select(
...
import_tea_data_cleanly <-
  function(urls = NULL, ...) {
    ret <-
      ...
      mutate(contents = purrr::map(path, ~import_tea_data(.x, ...))) %>%
      unnest() %>%
...
schools_tea <-
  urls_tea %>%
  import_tea_data_cleanly(grp_rgx = "All Students")
schools_tea
...
# path_save <- file.path(config$dir_data, "01.RData")
# session::save.session(file = path_save)
vars_all <- ls()
vars_keep <-
  vars_all %>% 
  str_subset("config$import")
vars_rm <- setdiff(vars_all, vars_keep)
rm(list = vars_rm)
teproj::export_ext_rdata(
  "01.RData",
  export = config$export_data,
  dir = config$dir_data
)
