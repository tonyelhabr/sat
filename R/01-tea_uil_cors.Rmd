...
[my series of posts about Texas high school UIL academic competitions](https://tonyelhabr.rbind.io/posts/analysis-texas-high-school-academics-intro/)
...
I was implying that the academic UIL scores may not correspond
well, or at all, with standardized test scores, but I did not attempt to prove so.
So now let's set out to see what
...
as well the well-known `{stringr}` and `{purrr}` packages in the `{tidyverse}`.
...
and cleaned for
[my series of posts analyzing the UIL data](https://tonyelhabr.rbind.io/posts/analysis-texas-high-school-academics-intro/))
