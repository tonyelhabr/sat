...
```{r urls_tea_dl, eval = config$eval_slow}
# NOTE(s):
...
```{r schools_tea, results = "hide", eval = config$eval_slow}
schools_tea <-
  urls_tea %>%
...
if(FALSE) {
  ...
  vars_keep <-
    vars_all %>% 
    str_subset("^config$|^urls_tea$|^create_path_tea$|^import|^schools_tea$|^schools_uil$")
